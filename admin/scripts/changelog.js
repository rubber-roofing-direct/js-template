...
import { getLongHash, getRevList, parseCommit } from "./parse-commit.js"
...
const getVersions = tag => {
    ...
    try {
        ...
        const msg = "Invalid version string format"
...
// Username (or organisation name) and repository name of repo on github.
let /** @type {string} */ user, /** @type {string} */ repo

// Fetch user and repo from the from the repository field of the package.json
// file. Regex for fetching user and repo are currently based on the github url
// format only since the template repository which this script is from is
// intended only for initialising boilerplate javascript repositories on github.
try {
    const regex = /^git\+https:\/\/github\.com\/(?<user>.*)\/(?<repo>.*)\.git/
    const packageJson = JSON.parse(fs.readFileSync("package.json").toString());
    ({ user, repo } = /** @type {{user:string, repo:string}} */
        (packageJson.repository?.url?.match(regex)?.groups || {}))
    if (!user || !repo) {
        throw new Error("Repository unset or not recognized in package.json")
    }
}
catch (error) {
    const msg = "No remote url found"
    error = decorateFg(/** @type {string} */ (error), "gray", 1)
    throw new Error(`${msg}\n${error}\n`)
}
...
await fetch(`https://github.com/${cli.user}/${cli.repo}`, { signal })
    .then(res => {
        controller.abort()
        if (!res.ok) {
            const msg = `Remote repository responded with "${res.statusText}"`
            const url = decorateFg(`url: ${res.url}`, "gray", 1)
            const status = decorateFg(`status: ${res.status}`, "gray", 1)
            throw new Error(`${msg}\n${url}\n${status}\n`)
        }
    })
...
try {
    ...
    const msg = "Error parsing existing changelog"
    const path = decorateFg(`path: ${process.cwd()}/${cli.path}`, "gray", 1)
    error = decorateFg(/** @type {string} */ (error), "gray", 1)
    throw new Error(`${msg}\n${path}\n${error}\n`)
}
...
for (const hash of revList) {
    ...
    if (commit.isRevert) {
        ...
        try {
            ...
            const msg = "Could not find reverted commit hash in commit summary"
            error = decorateFg(/** @type {string} */ (error), "gray", 1)
            throw new Error(`${msg}\n${error}\n`)
        }
    ...
    // versionFlag = commit.title.semver?.match(/breaking/i) ? versionFlag | 4
    //     : commit.title.semver?.match(/minor/i) ? versionFlag | 2
    //     : commit.title.semver?.match(/patch/i) ? versionFlag | 1
    //     : versionFlag
...
try {
    template = fs
        .readFileSync("./admin/templates/changelog.hbs")
        .toString()
